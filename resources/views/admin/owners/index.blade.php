                              ...
                              <tbody>
                                @foreach($owners as $owner)<tr>
                                  ...
                                  <td class="md:px-4 py-3">
                                    <button onclick="location.href='{{ route('admin.owners.edit', ['owner'=>$owner->id]) }}'" class=" text-white bg-green-300 border-0 py-2 px-4 focus:outline-none hover:bg-green-400 rounded">編集</button>
                                  </td>
                                  <form id="delete_{{ $owner->id }}" method="post" action="{{ route('admin.owners.destroy', ['owner'=>$owner->id]) }}">
                                    @csrf
                                    @method('delete')
                                  <td class="md:px-4 py-3">
                                    <a href="#" data-id="{{ $owner->id }}" onclick="deletePost(this)" class=" text-white bg-red-300 border-0 py-2 px-4 focus:outline-none hover:bg-red-400 rounded">削除</a>
                                  </td>
                                  </form>
                                @endforeach</tr>
                              </tbody>
